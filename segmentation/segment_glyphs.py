...
NUM_ROWS = 7
NUM_COLS = 10
MARGIN = 5 # To not accidentally crop a letter

# Loads the scanned template
template_path = os.path.join(SCRIPT_DIR, "handwritten_template.jpg")
...
output_folder = os.path.join(SCRIPT_DIR, "glyph_images") # Each cropped letter will be saved here
...
for row in range(NUM_ROWS):
    for col in range(NUM_COLS):
        ...
        x2 = (col + 1) * cell_width - MARGIN  # inset from right edge
        y2 = (row + 1) * cell_height - MARGIN  # inset from bottom edge

        glyph = img[y1:y2, x1:x2] # row_start:row_end, col_start:col_end

        # Threshold to isolate pen strokes (binarize)
